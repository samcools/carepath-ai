...
function findPatient(q: string, patients: Patient[]) {
  return patients.find(p => q.includes(`${p.firstName} ${p.surname}`.toLowerCase()))
    || patients.find(p => q.includes(p.firstName.toLowerCase()) && p.firstName.length > 4);
}
...
export async function answerAyanda(input: string, ctx: AyandaContext) {
  ...
  if (patient && (q.includes('open') || q.includes('record') || q.includes('history') || q.includes('summary'))) {
    ...
    return {
      intent: 'OPEN_PATIENT', navigate: 'patients', patientId: patient.id,
      text: `${patientLabel(patient)} is a synthetic demo patient. The authorised record currently shows ${condition}, ${med}, and ${allergy}. I can open the full OneRecord view with provenance.`
    };
  }
  ...
  if (q.includes('outcome') || q.includes('recover') || q.includes('died') || q.includes('mortality') || q.includes('best hospital') || q.includes('referral performance')) {
    return {
      intent: 'OPEN_OUTCOMES', navigate: 'outcomes',
      text: 'Opening Referral Outcomes. The hospital-level figures are synthetic observed outcomes, not risk-adjusted quality rankings. Production comparison would need case-mix adjustment, minimum sample sizes and validated outcome definitions.'
    };
  }
  ...
  if (q.includes('create') && q.includes('referral') && patient) {
    ...
    const candidate = ctx.facilities.find(f => f.services.some(s => s.toLowerCase().includes(service.toLowerCase()))) || ctx.facilities.find(f => f.type === 'PUBLIC');
    return {
      intent: 'PROPOSE_CREATE_REFERRAL', confirmationRequired: true,
      proposedAction: { tool: 'create_referral_draft', patientId: patient.id, service, destinationFacilityId: candidate?.id },
      text: `I can create a ${service} referral draft for ${patientLabel(patient)}${candidate ? ` to ${candidate.name}` : ''}. I will create a draft only; an authorised clinician must review and submit it. Confirm to continue.`
    };
  }
  ...
  return {
    intent: 'HELP',
    text: 'I can open a patient record, explain allergies or medication, show overdue referrals, open referral outcomes, navigate Care Gaps or Health Passport, and propose a governed referral draft. Try “open Thandi Mokoena”, “show referral outcomes”, or “show referrals waiting more than 24 hours”.'
  };
}
