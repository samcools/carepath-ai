...
async function ensureRoleConsole(){
  ...
  bar.querySelector<HTMLSelectElement>('#carepath-demo-user')?.addEventListener('change',async e=>{const select=e.currentTarget;select.disabled=true;try{await request('/api/auth/switch-demo',{method:'POST',body:JSON.stringify({username:select.value})});window.location.reload()}catch(err){select.disabled=false;alert(err instanceof Error?err.message:String(err))}});
}
